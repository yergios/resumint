...
const CONTACT_ORDER = ["email", "web", "phone", "github", "location", "linkedin"];
...
async function runSpellCheck(
  ...
  if (result.misspelledCount > 0) {
    log(generationResult, "warn", `Found ${result.misspelledCount} misspelled words in '${language}' resume:`);
...
function buildContactInfo(resumeData: ResumeData, language: string): ContactInfo[] {
  ...
  return [...resumeData.basic.contactInfo, { type: "location", value: locationValue }].sort(
...
export async function generateResumes(options: CommandLineArgs) {
  try {
    ...
    const templatePath = resolve(process.cwd(), options.templatesDir, `${templateName}-template.html`);
    ...
    if (!languages || languages.length === 0) {
      throw new Error("No languages specified in resume data or via --language");
    }
    ...
    await Promise.all(
      languages.map((language) => {
        ...
        const generationResult: GenerationResult = {
          ...
          html: template({ ...resumeData, language, basic: { ...resumeData.basic, contactInfo } }),
          logs: [],
          errors: [],
          success: true,
          metadata: {
